...
Auf die Programmiersprachen betrachtet, haben die mit Dart programmierten Flutter Applikationen die geringste Renderzeit, während die Auslastung des RAMs bei den Android Applikationen deutlich geringer war.
Die Implementierungen mit einem WebContainer, also die hybride und Cross-kompilierte hybride, schneiden dabei, trotz  


dass die die Android Apps und die reine Flutter App sehr vergleichbar sind, während die Hybride Flutter App in fast jeder Kategorie doppelt so viele Ressourcen verbraucht hat, wie die native Android Anwendungen.

Dies dürfte an der Art liegen wie Flutter seine UI aufbaut. Denn anstatt die nativen Elemente zu nutzen, setzt Flutter auf einen nativen Canvas, auf dem die Flutter eigene Rendering\TODO{Hier darauf eingehen war Flutter schneller die Oberfläche mit Canvas und dadurch besser als nativ}
